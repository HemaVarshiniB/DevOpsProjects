# 🚀 Deploy Static Website on AWS EC2
## This project demonstrates how to deploy a simple index.html file on an AWS EC2 instance using a web server.
...
Step8: If you find any errors in step7, then you might have to install webservers like http/apache2. ```sudo yum update -y```(for Amazon Linux AMI), ```sudo yum install httpd -y```, ```sudo systemctl start httpd```, ```sudo systemctl enable httpd```. Now if you ls /var/www/ you should see html. Now, sudo mv index.html /var/www/html
